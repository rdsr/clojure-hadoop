...
;;        -job clojure-hadoop.test.examples.wordcount5/job \
;;        -input README.txt -output out5
;;
;; The output is plain text, written to out5/part-r-00000
;;
...
(ns clojure-hadoop.test.examples.wordcount5
...
(imp/import-mapreduce)  ;; for TaskInputOutputContext
...
(defn string-long-writer
  [^TaskInputOutputContext context ^String key value]
  (.write context
          (Text. key)
          (LongWritable. value)))

(defn string-long-reduce-reader
  [^Text key wvalues]
  [(str key)
   (fn []
